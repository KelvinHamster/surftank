function bds = beachincident_IC(Nx,x0,a0,theta,varargin)
%BEACHINCIDENT_IC Sets up a bem_sim object with an initial condition for an overturn.
...
bds = cell(1,8);
bds{1} = free_surface_bdry('surface_nodes',[x_FS', eta']);
bds{2} = solid_bdry('surface_nodes',[linspace(0,0,lwallnodes)', linspace(0,-h0,lwallnodes)']);
bds{3} = solid_bdry('surface_nodes',[linspace(0,L0,deepnodes)', linspace(-h0,-h0,deepnodes)']);
bds{4} = solid_bdry('surface_nodes',[linspace(L0,L0+L1,slope1nodes)', linspace(-h0,-h1,slope1nodes)']);
bds{5} = solid_bdry('surface_nodes',[linspace(L0+L1,L0+L1+L2,barnodes)', linspace(-h1,-h1,barnodes)']);
bds{6} = solid_bdry('surface_nodes',[linspace(L0+L1+L2,L0+L1+L2+L3,slope2nodes)', linspace(-h1,-h2,slope2nodes)']);
bds{7} = solid_bdry('surface_nodes',[linspace(L0+L1+L2+L3,L,troughnodes)', linspace(-h2,-h2,troughnodes)']);
bds{8} = solid_bdry('surface_nodes',[linspace(L,L,rwallnodes)', linspace(-h2,0,rwallnodes)']);

bds{1}.characteristics.phi = phi_FS;
end
