function V1 = curve_renode_by_integral(V,integrand_nodes,numnodes,dim_scales)
%Creates a 3xnumnodes matrix representing a curve reweighed so that the integral between segments is close.
% the distance between two nodes should be chosen such that the integral
% between them of the interpolated integrand
% should be roughly the same.
%
%
%   V - 3xN matrices, where each column represents a point in 3d space
%
%   integrand_nodes - N length array where each entry is the integrand at
%                   the node
%
...
for j = 2:(numnodes-1)
    ...
    while right - left > len_tol
        ...
        ds = vecnorm(dr .* dim_scales,2,1);
        I = squeeze(integ_spline(k,:)) * (((LEN_QUAD_T*c).^(3:-1:0))');
        fc = c * dot(LEN_QUAD_W, ds .* I);
        if l < fc
